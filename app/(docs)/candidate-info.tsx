...
import PaymentModal from "@/components/PaymentModal"; // Ensure this is the correct path to PaymentModal
...
const CandiInfo: React.FC = () => {
  const { userId } = useLocalSearchParams();
  const { user } = useUser();
  const router = useRouter();
  const [resumeData, setResumeData] = useState<ResumeData | null>(null);
  const [loading, setLoading] = useState(true);
  const [showPaymentModal, setShowPaymentModal] = useState(true); // State to control payment modal visibility
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      <View className="absolute bottom-0 w-full mb-4 p-4">
        <CustomButton title="Start Conversation" onPress={startConversation} />
        <CustomButton
          title="Close"
